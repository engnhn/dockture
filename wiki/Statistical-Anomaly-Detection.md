...
in this equation, $x$ represents the current cpu percentage or memory byte sample, $\mu$ is the rolling mean across the historical window, $\sigma$ is the sample standard deviation, and $s$ is the `anomaly_sensitivity` floor parameter (default: `2.0`). clamping the denominator with $s$ prevents division by zero and eliminates false positive alerts when container resource usage remains flat ($\sigma \to 0$).

resource statistics are sampled every 30 seconds, maintaining a rolling buffer of up to 60 samples (a 30-minute historical window). a minimum of 15 samples (~7.5 minutes) must be collected before anomaly scoring begins. to prevent false alarms on low-usage idle containers, anomaly alerts are evaluated only if resource usage meets or exceeds the absolute minimum floor thresholds: `anomaly_min_value_cpu` (default: `25.0`%) for CPU or `anomaly_min_value_mem` (default: `40.0`%) for Memory. if a calculated z-score exceeds `anomaly_threshold` (default: `3.0`), dockture flags the sample as a resource anomaly, applies the `anomaly_cooldown_secs` timer (default: `1800`s / 30 minutes), and dispatches a `warning` category notification.
